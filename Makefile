...
DOCKER_CMD := docker run -it --rm --gpus=all --privileged=true --net=bridge --ulimit core=0 --ipc=host -v $(shell pwd):/app -v /home/jingyulee/data/coco2017:/data/coco2017
...
models/ssd300.fp32.b%.k256.plan:
	python ssd300_trt.py --mode=export --precision=fp32 --batch-dim=$* --topk=256 --runtime=trt

models/ssd300.fp16.b%.k256.plan:
	python ssd300_trt.py --mode=export --precision=fp16 --batch-dim=$* --topk=256 --runtime=trt

models/ssd300.int8.b%.k256.plan:
	python ssd300_trt.py --mode=export --precision=int8 --batch-dim=$* --topk=256 --runtime=trt

models: $(foreach batch_size, $(BATCH_SIZES), models/ssd300.fp32.b$(batch_size).k256.plan models/ssd300.fp16.b$(batch_size).k256.plan models/ssd300.int8.b$(batch_size).k256.plan)

### COCO evaluation

logs/ssd300.fp32.b%.k256.pytorch.coco: ssd300_trt.py
	python $< --mode=coco --precision=fp32 --batch-dim=$* --topk=256 --runtime=pytorch --output-path=$@
...
logs/ssd300.fp32.b%.k256.pytorch.bench: ssd300_trt.py
	python $< --mode=bench --precision=fp32 --batch-dim=$* --topk=256 --runtime=pytorch --output-path=$@
...
logs/ssd300.fp32.b%.k256.pytorch.qdrep: ssd300_trt.py
	nsys ${PROFILE_CMD} -o $@ python $< --mode=bench --precision=fp32 --batch-dim=$* --topk=256 --runtime=pytorch

logs/ssd300.fp16.b%.k256.pytorch.qdrep: ssd300_trt.py
	nsys ${PROFILE_CMD} -o $@ python $< --mode=bench --precision=fp16 --batch-dim=$* --topk=256 --runtime=pytorch

logs/ssd300.fp32.b%.k256.trt.qdrep: ssd300_trt.py models/ssd300.fp32.b16.k256.plan
	nsys ${PROFILE_CMD} -o $@ python $< --mode=bench --precision=fp32 --batch-dim=$* --topk=256 --runtime=trt

logs/ssd300.fp16.b%.k256.trt.qdrep: ssd300_trt.py models/ssd300.fp16.b16.k256.plan
	nsys ${PROFILE_CMD} -o $@ python $< --mode=bench --precision=fp16 --batch-dim=$* --topk=256 --runtime=trt

logs/ssd300.int8.b%.k256.trt.qdrep: ssd300_trt.py models/ssd300.int8.b16.k256.plan
	nsys ${PROFILE_CMD} -o $@ python $< --mode=bench --precision=int8 --batch-dim=$* --topk=256 --runtime=trt

qdrep: $(foreach batch_size, $(BATCH_SIZES), logs/ssd300.fp32.b$(batch_size).k256.pytorch.qdrep logs/ssd300.fp16.b$(batch_size).k256.pytorch.qdrep logs/ssd300.fp32.b$(batch_size).k256.trt.qdrep logs/ssd300.fp16.b$(batch_size).k256.trt.qdrep logs/ssd300.int8.b$(batch_size).k256.trt.qdrep)
